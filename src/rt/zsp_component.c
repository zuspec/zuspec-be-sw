...
static void zsp_component__do_init(zsp_executor_t *exec_b, zsp_component_t *self) {

    self->default_executor = exec_b;

    // Call init_down execs
    zsp_component_type(self)->init_down(exec_b, self);
    ...
    if (self->children) {
        ...
        while (c) {
            ...
            zsp_component_type(c)->do_init(exec_b, self);
    ...
    zsp_component_type(self)->init_up(exec_b, self);
}
