...
#****************************************************************************
"""Compare Zuspec generated C vs SystemC for a simple 1ns wait loop.

This mirrors the pattern in tests/unit/test_async_port_perf.py, but uses SystemC.

Usage:
  python3 tests/perf/compare_zuspec_c_vs_systemc.py --iterations 10000000
"""
...
@zdc.dataclass
class AsyncCounterPerf(zdc.Component):
    """Single-step async method used by the C benchmark."""

    async def count_1(self):
...
def run_zuspec_c(iterations: int, opt_flags: list[str]):
    with tempfile.TemporaryDirectory(prefix="zsp_perf_") as td:
        td_p = Path(td)

        dm_ctxt = zdc.DataModelFactory().build([AsyncCounterPerf])
...
#include "asynccounterperf.c" /* bring in static *_task symbols */
...
static zsp_frame_t *driver_task(zsp_thread_t *thread, int idx, va_list *args) {
    ...
    switch (idx) {
        case 0: {
            ret = zsp_timebase_alloc_frame(thread, sizeof(locals_t), &driver_task);
            locals_t *locals = zsp_frame_locals(ret, locals_t);
            if (args) {
                locals->self = (AsyncCounterPerf *)va_arg(*args, void *);
                locals->remaining = va_arg(*args, uint32_t);
            }
            ret->idx = 1;
        ...
        case 1: {
            ...
            if (locals->remaining == 0) {
                ...
                locals->remaining--;
                ret->idx = 1;
                ret = zsp_timebase_call(thread, &AsyncCounterPerf_count_1_task, locals->self);
            }
            break;
        }
    }
...
int main(int argc, char **argv) {
    ...
    AsyncCounterPerf comp;
    AsyncCounterPerf_init(&ctxt, &comp, "comp", NULL);

    double t0 = now_sec();
    zsp_timebase_thread_create(&tb, &driver_task, ZSP_THREAD_FLAGS_NONE, &comp, iterations);
    run_to_idle(&tb);
    double t1 = now_sec();
...
'''
        ...
        # main.c includes asynccounterperf.c so we must not compile it separately
        ...
        if rc != 0:
            raise RuntimeError(f"Zuspec C compile failed\nstdout:\n{out}\nstderr:\n{err}")

        rc, out, err = _run([str(exe), str(iterations)], cwd=str(td_p))
        if rc != 0:
            raise RuntimeError(f"Zuspec C run failed\nstdout:\n{out}\nstderr:\n{err}")
...
def run_systemc(iterations: int, opt_flags: list[str]):
    ...
    with tempfile.TemporaryDirectory(prefix="systemc_perf_") as td:
        td_p = Path(td)
...
def main():
    ...
    ap.add_argument("--min-runtime-sec", type=float, default=4.0,
                    help="Ensure each benchmark runs at least this long")
    ap.add_argument("--opt", default="-O3", help="Optimization flags (eg -O2, '-O3 -flto')")
    args = ap.parse_args()

    opt_flags = args.opt.split()
    if not opt_flags:
        opt_flags = ["-O3"]

    iters = int(args.iterations)
    min_s = float(args.min_runtime_sec)

    # Scale iterations up until both runs meet min runtime
    for _ in range(5):
        zsp_m, _ = run_zuspec_c(iters, opt_flags)
        sc_m, _ = run_systemc(iters, opt_flags)

        zsp_t = zsp_m.get("elapsed_sec") or 0.0
        sc_t = sc_m.get("elapsed_sec") or 0.0
        m = min(zsp_t, sc_t)

        if m >= min_s:
            break

        # Scale based on the faster (min elapsed) with a safety factor
        if m > 0.0:
            scale = (min_s / m) * 1.15
            iters = max(iters + 1, int(iters * scale))
        else:
            iters *= 10

    print("=" * 72)
    print(f"Iterations: {iters} (min-runtime-sec={min_s})")
    print("=" * 72)

    print("Zuspec C (generated):")
