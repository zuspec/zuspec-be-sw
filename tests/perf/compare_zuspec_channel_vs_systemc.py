...
#****************************************************************************
"""Compare Zuspec generated C vs SystemC for FIFO-channel get/put with blocking.

Goal: measure a steady-state pattern that does ~2 thread context switches per iteration:
  (1) consumer blocks waiting on get
  (2) producer runs put and unblocks consumer

The benchmark structure is based on the unit-test example in tests/unit/test_tlm_channel_codegen.py.

Usage:
  python3 tests/perf/compare_zuspec_channel_vs_systemc.py --iterations 10000000

Notes:
- Requires SystemC; set SYSTEMC_HOME if not installed in the default location.
"""
...
class Producer(zdc.Component):
    ...
    async def send_1(self):
        # One send "step" used by the producer process task
        await self.p.put(0)
...
class Consumer(zdc.Component):
    ...
    async def recv_1(self):
        # One receive "step" invoked by the benchmark driver task
        await self.c.get()
...
def run_zuspec_c(iterations: int, opt_flags: list[str]):
    with tempfile.TemporaryDirectory(prefix="zsp_chperf_") as td:
        ...
        # main.c includes the generated *.c so we can access static task symbols
        main_c = r'''
...
/* Bring in generated implementation (for static task symbols, if any) */
#include "producer.c"
#include "consumer.c"
...
/* Producer process: call generated Producer_send_1 (put + wait(1ns)) */
    ...
    switch (idx) {
        ...
        case 1: {
            ...
            if (locals->remaining == 0) {
                ...
                locals->remaining--;
                ret->idx = 1;
                ret = zsp_timebase_call(thread, &Producer_send_1_task, &locals->top->p);
            }
            break;
        }
    }
...
/* Consumer driver: test calls recv_1 (which blocks on get) */
    ...
    switch (idx) {
        ...
        case 1: {
            ...
            if (locals->remaining == 0) {
                ...
                locals->remaining--;
                ret->idx = 2;
                ret = zsp_timebase_call(thread, &Consumer_recv_1_task, &locals->top->c);
            }
            break;
        }
        case 2: {
...
int main(int argc, char **argv) {
    ...
    /* Consumer count is returned via its thread->rval; easiest: print sim-time and elapsed */
